...
servo_pulse_width_to_count_multiplier = (
    1.0 / servo_pulse_duration_us * 4095.0
...
def main():
    ...
    angle_range = np.array([-90.0, 90.0])
    ...
    # which servo?
    servo_num = 0
    angles = angle_range[0] + np.array([0.4, 0.6]) * (
    ...
    dt_traj = 1.0 / servo_pulse_frequency
    # Time array for the trajectory
    traj_time = np.linspace(0.0, t_max, np.int(np.ceil(t_max / dt_traj)))
    n = len(traj_time)
    # ANGLES array for the trajectory
    traj_ang = np.linspace(-90.0, 90.0, n)
